...
class Reachability3D:
    ...
    def center_R_A(self, t):
        ...
        def integrand(s):
            return expm(self.A * (t - s)) @ np.array([0, 0])
    ...
    def visualize(self):
        ...
        for t in time_points:
            ...
            boundary_A = np.array([deformation_A * psi + shape_A.get_center() for deformation_A, psi in zip(deformations_A, directions)])
            x_A, y_A = boundary_A[:, 0], boundary_A[:, 1]
            z_A = np.full_like(x_A, t)
            ax.plot(z_A, x_A, y_A, color="green", alpha = 0.7)
            ...
            x_B, y_B = boundary_B[:, 0], boundary_B[:, 1]
            z_B = np.full_like(x_B, t)
            ax.plot(z_B, x_B, y_B, color="red", alpha = 0.7)
...
def main():
    A = [[0.2, 0], [0, 0.2]]
    B = [[0, -0.5], [0.5, 0]]
    circle1 = CircleShape(center=[0, 5], radius=2.0)
    circle2 = CircleShape(center=[-25, 0], radius=1.0)
    square = SquareShape(center=[0, 0], side_length=2.0)
    mink_sum = MinkowskiSum(circle1, square)

    F_radius_A = 2.0
    F_radius_B = 1.0
    t_span = [0, 2 * np.pi]

    system = Reachability3D(A, circle1, F_radius_A, B, circle2, F_radius_B, t_span)
    system.visualize()
